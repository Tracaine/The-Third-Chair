...
**Interfaces:**
- Consumes: `NarratorInput` only.
- Produces: `OpenAiNarratorAdapter implements NarratorPort`, `validateNarration`, `renderTerseNarration`, and a Bill-owned recovery decision after repeated narration failure.

- [ ] **Step 1: Write failing tool and spoiler tests**

Assert the Narrator agent has `tools: []`, receives no Director context, includes every visible resolution ID and required event ID, emits the check block before its consequence, and rejects sentinel secrets, invented Raven dialogue, invented Bill dialogue, inner thoughts, extra actions, or resource spends.

- [ ] **Step 2: Run tests to verify RED**

Run: `npm test -- packages/agents/test/narrator*.test.ts packages/engine/test/turn/narrator-failure.test.ts`
...
After two Narrator failures, store the turn as `AWAITING_INPUT` and return a Bill-owned clarification: `Use terse deterministic rendering for this already-resolved turn?` If Bill accepts through a new idempotent command, `renderTerseNarration()` formats visible events and checks from code, then commits the original candidate. It does not invoke the Director or reroll.

- [ ] **Step 6: Run tests and commit**

Run: `npm test -- packages/agents/test/narrator*.test.ts packages/engine/test/turn/narrator-failure.test.ts && npm run typecheck`
