...
export function createHttpApp(server, fakeMode, sdkServer) {
    const app = express();
    ...
    if (sdkServer) {
        ...
        app.all("/mcp", async (request, response, next) => { try {
        ...
        app.post("/mcp", async (request, response, next) => { try {
            response.json(await server.invoke(String(request.body?.name), request.body?.arguments));
        }
        catch (error) {
            next(error);
        } });
