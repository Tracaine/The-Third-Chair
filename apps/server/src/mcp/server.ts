...
export function createMcpServer(deps: ServerDependencies): McpServer {
  return { tools: [listCampaignsDescriptor, getTableViewDescriptor, advanceGameDescriptor, answerRulesDescriptor, recallKnownLoreDescriptor], async invoke(name, input) {
...
export function createSdkMcpServer(deps: ServerDependencies): SdkMcpServer {
