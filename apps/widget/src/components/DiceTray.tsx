...
export function DiceTray({ checks }: DiceTrayProps) {
        ...
        {recentChecks.map((check) => (
          <li key={check.id} className="check-card">
            <div className="check-card__heading">
              <div><span className="check-card__actor">{check.checkKind}</span><strong>{check.keptDie} {check.modifier >= 0 ? "+" : "−"} {Math.abs(check.modifier)} = {check.total}</strong></div>
