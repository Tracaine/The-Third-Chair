...
The current private build provides Raven's Table as a ChatGPT developer-mode plugin: a live GPT-5.6 Sol Director and Narrator, persistent SQLite campaigns, ten player-safe MCP tools, instant two-seat quickstart characters, and a stateful table widget.
...
When the launcher says Raven's Table is ready, it copies the `tunnel_id` to the clipboard. In ChatGPT, enable Developer mode under **Settings → Security and login**, open **Plugins**, select the plus button, choose **Tunnel** under Connection, and select the tunnel or paste its ID. Create the plugin, review its ten discovered tools, and add it to a new conversation. Ask to start a new adventure; Bill chooses Bill's archetype and Raven chooses hers, then play begins at the opening decision.
