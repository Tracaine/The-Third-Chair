...
Prerequisites: Node 24, the private source pack at `private/source-pack.sqlite`, and `OPENAI_API_KEY` in the current PowerShell environment or the ignored root `.env` file. For the private ChatGPT connection, create a tunnel in [OpenAI Platform Tunnels](https://platform.openai.com/settings/organization/tunnels) and a restricted runtime API key with Tunnels **Read** + **Use**.
...
The launcher builds the widget, starts the live server on loopback, downloads OpenAI's current tunnel runtime when needed, and verifies the private connection. If the tunnel values are not already in the ignored `.env` file or PowerShell environment, it prompts for the `tunnel_id` and runtime key. The runtime key is hidden while you paste it and is not saved by the launcher.
