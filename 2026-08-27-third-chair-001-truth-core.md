...
- Produces: `IdSchemas`, `ActorIntentSchema`, `DecisionRequestSchema`, `WorldStateSchema`, `WorldState`, `requiredSeats(decision)`, and `validateIntentsForDecision(decision, intents)`.
...
it("rejects a BOTH decision with only Bill's intent", () => {
  expect(() => validateIntentsForDecision(bothDecision, [billIntent])).toThrow(
...
it("rejects a Raven intent for Bill's actor", () => {
  expect(() => validateIntentsForDecision(ravenDecision, [{ ...ravenIntent, actorId: "actor_bill" }]))
...
Define `ActorStateSchema` with controller, name, level, class/ancestry/background source keys, abilities, proficiency bonus, AC, max/current/temporary HP, speed, conditions, death saves, resources, spells, equipment IDs, public notes, and audience-scoped notes. Define the other nested schemas with IDs, bounded names/statuses, and audience-scoped fact arrays; do not add unscoped `secret`, `notes`, `plan`, or `intent` string fields.

- [ ] **Step 5: Implement decision validation and verify GREEN**

`requiredSeats()` maps `BILL` to `{"BILL"}`, `RAVEN` to `{"RAVEN"}`, `BOTH` to both seats, and `DIRECTOR` to an empty set. `validateIntentsForDecision()` rejects duplicate seats, missing required seats, extra seats, ineligible actors, mismatched controllers, and any `ACT` that commits a resource ID the actor does not own.
...
**Interfaces:**
- Consumes: `WorldState`, `DecisionRequest`, `ActorIntent`, `TurnStatus`.
- Produces: `openCampaignDatabase(path)`, `runMigrations(db)`, `CampaignRepository`, `TurnRepository`, `BeginTurnResult`, and `CommitTurnInput`.

- [ ] **Step 1: Write failing migration and compare-and-set tests**

Tests must prove WAL and foreign keys are enabled for file databases, migrations run once, request IDs are unique per campaign, state version conflicts do not insert turns, and a second database instance can resume `PLANNED` and `RESOLVED` turns.
...
Insert the root branch and campaign in one transaction. Defer the foreign key from `campaigns.active_branch_id` because SQLite cannot add it cleanly during the initial mutual insert; repository validation must ensure it names a branch in the same campaign.
...
export interface TurnRepository {
  ...
  persistResolutions(turnId: TurnId, resolutions: CheckResolution[], rngCounter: number): void;
...
`commitTurn()` uses `BEGIN IMMEDIATE`, reparses both state blobs with `WorldStateSchema`, verifies before-state hash and current campaign version, updates `campaigns` with `state_version + 1`, marks the turn `COMMITTED`, inserts the status event, and commits. On any error it rolls back. `markFailed()` never changes the campaign row.

- [ ] **Step 5: Prove restart and atomicity, then commit**

Run: `npm test -- packages/storage/test && npm run typecheck`

Expected: PASS. Kill and reopen file-backed test databases between stage writes; persisted plan/resolution records remain readable and campaign state remains unchanged before commit.
...
- Produces: `createTurnEngine(deps): TurnEngine`, `FakeDirector`, `FakeNarrator`, and `FailureInjector`.
...
Run the same command twice concurrently and expect one Director call, one Narrator call, one roll set, and one committed version. Inject failures after `PROCESSING`, `PLANNED`, `RESOLVED`, candidate validation, and narration. Before commit, the campaign state hash and version must remain unchanged. Recreate the engine and repositories from disk, call `resume(turnId)`, and expect stored plan and dice to be reused. Add a combat fixture in which Director-controlled actors advance internally until Raven owns initiative; assert the engine returns a Raven decision without inventing Raven's action, and a later Bill-owned initiative stops immediately for Bill.
...
Use a keyed in-process mutex to reduce duplicate work and repository compare-and-set as the cross-process authority. The exact pipeline is:

```ts
validate command -> lock campaign -> begin/resume turn -> build DirectorInput
-> DirectorPort.propose(runtime.lockAndResolveChecks) -> validate proposal
-> apply operations to clone -> project before/after views
-> NarratorPort.narrate -> validate narration -> repository.commitTurn
...
`NarrationSchema` contains `sceneText`, `spokenNpcLines`, `mustIncludeResolutionIds`, `mustIncludeEventIds`, and `visibleEventIds`. It cannot carry operations. The engine verifies every visible resolution and required visible event is represented before commit. Before projection and narration, the candidate state's `metadata.rngCounter` is set to the persisted next counter from the locked resolutions.
