...
describe("resumable exploration turn", () => {
  it("commits a BOTH decision once with code-owned metadata", async () => {
    ...
    try {
      const state = structuredClone(minimumWorldState); const campaignId = state.metadata.campaignId;
      ...
      const command = { kind: "INTENTS" as const, campaignId, expectedStateVersion: 0, decisionId: state.currentDecision.id, clientRequestId: "test_request_engine", intents: [{ seat: "BILL" as const, actorId: "test_actor_bill", mode: "ACT" as const, declaredAction: "Distract", desiredOutcome: "Time", approach: "Talk", committedResourceIds: [], targetIds: [], contingency: "Run" }, { seat: "RAVEN" as const, actorId: "test_actor_raven", mode: "ACT" as const, declaredAction: "Search", desiredOutcome: "Clue", approach: "Carefully", committedResourceIds: [], targetIds: [], contingency: "Stop" }] };
      const result = await engine.advanceGame(command);
      expect(result.kind).toBe("COMMITTED"); expect(result.view.stateVersion).toBe(1); expect(result.view.currentDecision.stateVersion).toBe(1); expect(result.view.currentDecision.owner).toBe("BILL");
      expect(createCampaignRepository(temp.db).getCampaign(campaignId).currentState.metadata.rngCounter).toBe(1);
      const retry = await engine.advanceGame(command); expect(retry.turn.id).toBe("test_turn_engine"); expect(director.calls).toBe(1); expect(narrator.calls).toBe(1);
