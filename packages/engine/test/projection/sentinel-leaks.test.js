...
describe("projection leak guards", () => {
    ...
    test("finds forbidden sentinels nested in arrays rather than relying on field names", () => {
        expect(() => assertNoForbiddenSentinels({ harmlessName: [{ detail: "SENTINEL_DIRECTOR_CLOCK" }] }, ["SENTINEL_DIRECTOR_CLOCK"])).toThrow("SENTINEL_DIRECTOR_CLOCK");
    });
    test("rejects structural secret field names at every nesting level", () => {
        expect(() => assertNoForbiddenSentinels({ public: [{ director: "not a sentinel" }] }, [])).toThrow("director");
    });
});
//# sourceMappingURL=sentinel-leaks.test.js.map