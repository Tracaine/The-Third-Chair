import { AdvanceGameCommandSchema, CheckResolutionSchema, ResolutionPlanSchema, TurnProposalSchema, } from "@third-chair/contracts";
...
import { NarrationSchema } from "./ports.js";
function nextId() { return `test_turn_${crypto.randomUUID().replaceAll("-", "_")}`; }
function parseResolutions(turn) { return (turn.resolutions ?? []).map((value) => CheckResolutionSchema.parse(value)); }
export function createTurnEngine(deps) {
    const mutex = new KeyedMutex();
    return { async advanceGame(raw) {
            ...
            return mutex.run(command.campaignId, async () => {
                ...
                let resolved = parseResolutions(turn);
                let nextRngCounter = turn.nextRngCounter;
                const lockAndResolveChecks = (rawPlan) => {
                    ...
                    if (turn.resolutionPlan !== null) {
                        ...
                        if (sha256Json(stored) !== sha256Json(plan))
                            throw new Error("RESOLUTION_PLAN_ALREADY_LOCKED");
                        if (turn.resolutions === null || turn.nextRngCounter === null)
                            throw new Error("LOCKED_PLAN_UNRESOLVED");
                    ...
                    nextRngCounter = result.nextRngCounter;
                    return result;
                };
                ...
                if (expectedResolutions.some((id) => !narration.mustIncludeResolutionIds.includes(id)))
                    throw new Error("NARRATION_MISSING_RESOLUTION");
...
//# sourceMappingURL=turn-engine.js.map