import { PlayerViewSchema, } from "@third-chair/contracts";
...
export function assertNoForbiddenSentinels(value, sentinels) {
    const visit = (candidate) => {
        if (typeof candidate === "string") {
            const found = sentinels.find((sentinel) => candidate.includes(sentinel));
            if (found)
                throw new Error(`Forbidden sentinel in player view: ${found}`);
            return;
        }
        ...
        if (candidate !== null && typeof candidate === "object") {
            for (const [key, nested] of Object.entries(candidate)) {
                if (forbiddenKeys.has(key))
                    throw new Error(`Forbidden player view key: ${key}`);
                visit(nested);
            }
        }
    };
...
export function projectPlayerView(state, viewer) {
    ...
    if (!location || !allowed.has(location.audience))
        throw new Error("Current location is not visible to player");
    const view = {
