...
function clearSidecars(path) {
    rmSync(`${path}-wal`, { force: true });
    rmSync(`${path}-shm`, { force: true });
}
export function createPreMigrationBackup(databasePath) {
...
export function restorePreMigrationBackup(databasePath, backupPath) {
    validateDatabaseFile(backupPath);
    clearSidecars(backupPath);
    const restoreStage = join(dirname(databasePath), `.${basename(databasePath)}.${randomUUID()}.restore`);
    try {
