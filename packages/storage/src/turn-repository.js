import { ActorIntentSchema, CheckResolutionSchema, DecisionRequestSchema, ResolutionPlanSchema, TurnProposalSchema, WorldStateSchema, validateIntentsForDecision, } from "@third-chair/contracts";
...
function parseTurn(row) {
    let resolutions = null;
    let nextRngCounter = null;
    if (row.resolutions_json !== null) {
        const stored = JSON.parse(row.resolutions_json);
        ...
        nextRngCounter = stored.nextRngCounter;
    }
    return {
        id: row.id,
...
class SqliteTurnRepository {
    ...
    beginTurn(input) {
      ...
      `).get(input.campaignId);
            ...
            if (campaign.state_version !== input.expectedStateVersion
            ...
            validateIntentsForDecision(currentDecision, lockedIntents, beforeState);
            if (campaign.active_branch_id !== input.branchId)
                throw new Error("ACTIVE_BRANCH_MISMATCH");
            const branch = this.db.prepare("SELECT campaign_id, status FROM branches WHERE id = ?").get(input.branchId);
      ...
      `).run(input.turnId, input.campaignId, input.branchId, input.clientRequestId, input.expectedStateVersion, input.decisionId, input.inputHash, JSON.stringify(beforeState), campaign.current_state_hash, JSON.stringify(lockedIntents), input.modelProfile === undefined ? null : stringifyJson(input.modelProfile), createdAt, createdAt);
            this.db.prepare(`
        INSERT INTO active_turns(
          campaign_id, turn_id, reserved_state_version, reserved_decision_id, reserved_at
        ) VALUES (?, ?, ?, ?, ?)
      `).run(input.campaignId, input.turnId, input.expectedStateVersion, input.decisionId, createdAt);
    ...
    persistPlan(turnId, plan) {
        const result = this.db.prepare(`
      UPDATE turns SET resolution_plan_json = ?, status = 'PLANNED', updated_at = ?
      WHERE id = ? AND status = 'PROCESSING'
    `).run(stringifyJson(ResolutionPlanSchema.parse(plan)), new Date().toISOString(), turnId);
        if (Number(result.changes) !== 1)
            throw transitionError();
    }
    persistResolutions(turnId, resolutions, rngCounter) {
        if (!Number.isSafeInteger(rngCounter) || rngCounter < 0)
            throw new Error("INVALID_RNG_COUNTER");
    ...
    persistProposal(turnId, proposal, candidate) {
        const parsedCandidate = WorldStateSchema.parse(candidate);
        const result = this.db.prepare(`
      UPDATE turns
      SET director_proposal_json = ?, candidate_state_json = ?, updated_at = ?
      WHERE id = ? AND status = 'RESOLVED'
    `).run(stringifyJson(TurnProposalSchema.parse(proposal)), JSON.stringify(parsedCandidate), new Date().toISOString(), turnId);
        if (Number(result.changes) !== 1)
            throw transitionError();
    }
    markAwaitingInput(turnId, decision) {
    ...
    `).run(stringifyJson(failure), new Date().toISOString(), turnId);
        if (Number(result.changes) !== 1)
            throw transitionError();
    }
    abandonTurn(turnId, failure) {
        ...
        UPDATE turns SET status = 'FAILED', error_json = ?, updated_at = ?
        WHERE id = ? AND status <> 'COMMITTED'
      `).run(stringifyJson(failure), now, turnId);
            if (Number(result.changes) !== 1)
                throw transitionError();
    ...
    commitTurn(input) {
        ...
        if (input.candidateStateHash.length === 0)
      ...
      `).get(turn.campaignId);
            ...
            const currentDecision = DecisionRequestSchema.parse(JSON.parse(campaign.current_decision_json));
            if (turn.beforeStateHash !== campaign.current_state_hash)
                throw new Error("BEFORE_STATE_HASH_MISMATCH");
            if (campaign.state_version !== turn.expectedStateVersion
      ...
      `).get(turn.campaignId);
            if (reservation?.turn_id !== turn.id)
                throw new Error("ACTIVE_RESERVATION_OWNER_MISMATCH");
            if (reservation.reserved_state_version !== turn.expectedStateVersion
                || reservation.reserved_decision_id !== turn.decisionId) {
                throw new Error("ACTIVE_RESERVATION_MISMATCH");
            }
            const branch = this.db.prepare("SELECT campaign_id FROM branches WHERE id = ?").get(turn.branchId);
            if (branch?.campaign_id !== turn.campaignId)
                throw new Error("BRANCH_OWNERSHIP_MISMATCH");
            const committedVersion = campaign.state_version + 1;
            if (candidate.metadata.campaignId !== turn.campaignId)
                throw new Error("CANDIDATE_CAMPAIGN_MISMATCH");
            if (candidate.metadata.stateVersion !== committedVersion)
                throw new Error("CANDIDATE_VERSION_MISMATCH");
            if (candidate.metadata.turnNumber !== beforeState.metadata.turnNumber + 1) {
                throw new Error("TURN_NUMBER_MISMATCH");
            }
            if (candidate.metadata.rngCounter !== turn.nextRngCounter)
                throw new Error("RNG_COUNTER_MISMATCH");
            if (candidate.currentDecision.stateVersion !== committedVersion
      ...
      `).run(committedVersion, JSON.stringify(candidate), input.candidateStateHash, JSON.stringify(nextDecision), committedAt, turn.campaignId, turn.expectedStateVersion, turn.beforeStateHash);
            if (Number(updatedCampaign.changes) !== 1)
                throw new Error("CAMPAIGN_COMPARE_AND_SET_FAILED");
      ...
      `).run(narrationJson, JSON.stringify(nextDecision), committedVersion, committedAt, turn.id);
            if (Number(updatedTurn.changes) !== 1)
                throw transitionError();
            this.db.prepare(`
        INSERT INTO turn_events(turn_id, status, payload_hash, created_at)
        VALUES (?, 'COMMITTED', ?, ?)
      `).run(turn.id, input.candidateStateHash, committedAt);
            const released = this.db.prepare("DELETE FROM active_turns WHERE campaign_id = ? AND turn_id = ?").run(turn.campaignId, turn.id);
            if (Number(released.changes) !== 1)
                throw new Error("ACTIVE_RESERVATION_RELEASE_FAILED");
    ...
    findByRequest(campaignId, clientRequestId) {
        const row = this.db.prepare("SELECT * FROM turns WHERE campaign_id = ? AND client_request_id = ?").get(campaignId, clientRequestId);
        return row ? parseTurn(row) : null;
    }
}
export function createTurnRepository(db) {
    return new SqliteTurnRepository(db);
}
//# sourceMappingURL=turn-repository.js.map