...
function pragmaNumber(db: DatabaseSync, sql: string, key: string): number {
  const row = db.prepare(sql).get() as Record<string, unknown> | undefined;
  const value = row?.[key];
  if (typeof value !== "number") throw new Error(`DATABASE_PRAGMA_VERIFICATION_FAILED:${key}`);
  return value;
}

export function openCampaignDatabase(path: string): DatabaseSync {
  if (path.trim().length === 0) throw new Error("DATABASE_PATH_REQUIRED");
...
export function verifyDatabase(db: DatabaseSync): void {
  ...
  if (integrity?.integrity_check !== "ok") throw new Error("DATABASE_INTEGRITY_CHECK_FAILED");
  const foreignKeyViolations = db.prepare("PRAGMA foreign_key_check").all();
  if (foreignKeyViolations.length > 0) throw new Error("DATABASE_FOREIGN_KEY_CHECK_FAILED");
}

export function checkpointCampaignDatabase(db: DatabaseSync): void {
