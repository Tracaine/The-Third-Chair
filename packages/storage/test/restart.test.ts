...
describe("turn restart recovery", () => {
  it.each([
    ["PROCESSING", (_repo: ReturnType<typeof createTurnRepository>, _turnId: string): void => undefined],
    ["PLANNED", (repo: ReturnType<typeof createTurnRepository>, turnId: string): void => {
      repo.persistPlan(turnId, resolutionPlan(turnId));
    }],
    ["RESOLVED", (repo: ReturnType<typeof createTurnRepository>, turnId: string): void => {
    ...
    ["AWAITING_INPUT", (repo: ReturnType<typeof createTurnRepository>, turnId: string): void => {
      repo.markAwaitingInput(turnId, decision("awaiting", 0));
    }],
  ] as const)("reopens and resumes a persisted %s turn", (status, stage) => {
