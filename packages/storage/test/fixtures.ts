...
export const billIntent: ActorIntent = {
  seat: "BILL",
...
export function decision(suffix: string, stateVersion = 0): DecisionRequest {
...
export function worldState(suffix: string, stateVersion = 0): WorldState {
...
export function seedCampaign(
  db: ReturnType<typeof openCampaignDatabase>,
  suffix: string,
) {
  const state = worldState(suffix);
  const campaignId = state.metadata.campaignId as CampaignId;
...
export function beginInput(suffix: string): BeginTurnInput {
  return {
...
export function resolutionPlan(suffix: string): ResolutionPlan {
  return {
...
export function turnProposal(nextDecision: DecisionRequest): TurnProposal {
  return {
...
export function committedCandidate(
  before: WorldState,
  suffix: string,
  rngCounter: number,
) {
