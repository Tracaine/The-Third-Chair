...
import { AudienceSchema, PersistedIdSchema, SeatSchema } from "./ids.js";
...
export function requiredSeats(decision) {
    if (decision.owner === "BILL")
        return new Set(["BILL"]);
    if (decision.owner === "RAVEN")
        return new Set(["RAVEN"]);
    if (decision.owner === "BOTH")
        return new Set(["BILL", "RAVEN"]);
    return new Set();
}
export function validateIntentsForDecision(decision, intents, currentState) {
    const parsedDecision = DecisionRequestSchema.parse(decision);
    const parsedIntents = z.array(ActorIntentSchema).parse(intents);
    const suppliedSeats = new Set();
    for (const intent of parsedIntents) {
        if (suppliedSeats.has(intent.seat))
            throw new Error(`Duplicate intent for ${intent.seat}`);
    ...
    if (extra.length > 0)
        throw new Error(`Decision does not accept intents from ${extra.join(" and ")}`);
    for (const intent of parsedIntents) {
        ...
        if (!actor)
            throw new Error(`Actor ${intent.actorId} does not exist in current state`);
