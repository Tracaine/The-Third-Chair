...
State, turn ledger, RNG counter, knowledge changes, memories, narration, and checkpoint metadata commit in one SQLite transaction. A model, validation, or narration failure leaves the previous committed reality authoritative.

### INV-008 - Retries do not grant new dice

Every mutating request includes a client request ID and expected state version. A retry returns or resumes the same turn record and reuses locked rolls. It never silently creates another attempt.
...
3. Acquire the campaign mutex and enforce the idempotency key.
4. Insert or resume a `PROCESSING` turn record with the before-state snapshot and locked intents.
...
12. Validate that the narration does not claim forbidden facts, invent player decisions, or contradict must-include results.
13. Atomically commit the candidate state, narration, ledger, RNG counter, memories, and next `DecisionRequest`.
14. Return a `TurnView` containing locked intents, visible checks, narration, player status, and the next decision owner.

The service never holds a SQLite transaction open during a model call. The turn record carries the workflow through `PROCESSING`, `PLANNED`, `RESOLVED`, `COMMITTED`, `FAILED`, or `AWAITING_INPUT` states; only the final commit changes campaign reality.
...
The schema permits an `AWAITING_ROLLS` state and a later `submit_rolls` operation, but the first playable release uses server-open dice by default. Manual dice is implemented only after the ordinary turn loop passes beta acceptance.
...
OCR text below the confidence threshold remains searchable but is marked `LOW_CONFIDENCE`; it cannot supply an exact proper noun, number, or rules-like claim without corroboration.
...
| Tool | Purpose | Mutation and retry behavior |
| --- | --- | --- |
| `list_campaigns` | List Bill's campaigns and latest visible status | Read-only, idempotent |
| `create_campaign` | Create a campaign, source-pack binding, party, and initial hidden spine | Mutating; idempotent by request ID |
| `get_table_view` | Return the current `PlayerView` and decision owner | Read-only, idempotent |
| `advance_game` | Submit the intents required by the current decision and resolve until the next player-owned decision | Mutating; idempotent by request ID and state version |
| `answer_rules` | Answer a player-safe SRD question with concise citation and any campaign house rule | Read-only, idempotent |
| `recall_known_lore` | Answer only from the selected actor's established knowledge and visible sources | Read-only, idempotent |
| `create_checkpoint` | Name and preserve the current committed state | Mutating, idempotent by request ID |
| `rewind_to_checkpoint` | Replace current state with a named checkpoint while preserving later history as an abandoned branch | Mutating and destructive; explicit confirmation required |
| `render_table` | Attach or refresh the persistent table widget using a current player-view ID | Read-only render tool |
| `export_campaign` | Produce a portable SaveSet without source text | Creates an export artifact; idempotent for a state version |

Every tool has an explicit input and output schema, accurate MCP annotations, and stable IDs. Mutating tools use `readOnlyHint: false`, `openWorldHint: false`, and state their reversibility. `rewind_to_checkpoint` uses `destructiveHint: true`. Tools returning structured content declare an output schema.
...
`export_campaign` creates a portable archive containing:
...
| Narrator failure | Retry Narrator against the same visible candidate; do not commit until narration succeeds or Bill chooses a terse deterministic rendering |
...
- Database migrations make a backup before mutation.
...
4. **Idempotency:** repeated request IDs return one committed result and one set of dice.
...
7. The campaign exports and reimports against the same source-pack hash.
